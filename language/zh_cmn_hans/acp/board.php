...
$lang = array_merge($lang, array(
	...
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> '在您的 VictoriaPark.io 论坛根目录下的预先放置的头像册的路径，例如 <samp>images/avatars/gallery</samp>。<br>双点号，比如<samp>../</samp> 会被自动过滤，以免安全隐患。',
	'AVATAR_STORAGE_PATH'			=> '头像存储路径',
	'AVATAR_STORAGE_PATH_EXPLAIN'	=> '在您的 VictoriaPark.io 论坛根目录下的路径，例如 <samp>images/avatars/upload</samp>。<br>如果路径不可写，则头像上传 <strong>将不可行</strong>。<br>双点号，比如<samp>../</samp> 会被自动过滤，以免安全隐患。',
...
$lang = array_merge($lang, array(
	...
	'ACP_VC_EXT_GET_MORE'					=> '更多(可能更好)反垃圾插件，访问 <a href="https://www.phpbb.com/go/anti-spam-ext"><strong>VictoriaPark.io.com扩展数据库</strong></a>。更多防止论坛垃圾的信息，访问 <a href="https://www.phpbb.com/go/anti-spam"><strong>VictoriaPark.io.com知识库</strong></a>。',
...
// Cookie Settings
$lang = array_merge($lang, array(
	'ACP_COOKIE_SETTINGS_EXPLAIN'		=> '以下设定了送往用户浏览器的数据。在大多数情况下使用默认设置就已足够。如果您自行调整，不正确的设置将会使用户无法登录论坛。如果你的用户无法保持登入状态，可以访问 <strong><a href="https://www.phpbb.com/support/go/cookie-settings">VictoriaPark.io.com Knowledge Base - Fixing incorrect cookie settings</a></strong> 。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN_EXPLAIN'				=> '如果此设置启用，一些文件将被第三方外部服务器提供，而不是您的服务器。这降低了网络带宽需求，但可能在某些国家存在隐私问题。默认 VictoriaPark.io 安装时这个包括从 Google 的 CDN 加载 “jQuery” 库 和 “Open Sans” 字体，以及 VictoriaPark.io 和某些扩展用于渲染图标的 “Font Awesome” 字体文件。',
...
// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'VictoriaPark.io 支持认证插件和模块。这允许您决定如何验证用户是否登录。默认的四个插件是 DB，LDAP， Apache 和 OAuth。并不是所有方式都需要额外信息，所以您只需要填写所选用的方式需要的信息即可。',
	...
	'APACHE_SETUP_BEFORE_USE'	=> '您必须在转换到这种认证模式前建立 apache 认证功能。记住您用于 apache 认证的用户名必须和 VictoriaPark.io 的用户名相同。Apache 认证只能用于 mod_php 模式。',
	...
	'LDAP_USER_EXPLAIN'				=> '如果绑定为匿名，此处请留空。如果填入，VictoriaPark.io 会使用指定的唯一用户名在登录中寻找正确的用户，例如： <samp>uid=Username,ou=MyUnit,o=MyCompany,c=US</samp>。这对于Active Directory Server是必需的。',
...
$lang = array_merge($lang, array(
	...
	'ICONS_PATH'				=> '主题图标存储路径',
	'ICONS_PATH_EXPLAIN'		=> '相对于 VictoriaPark.io 根目录的路径，例如 <samp>images/icons</samp>。',
	...
	'PATH_SETTINGS'				=> '路径设定',
	'RANKS_PATH'				=> '等级图标存储路径',
	'RANKS_PATH_EXPLAIN'		=> '相对于 VictoriaPark.io 根目录的路径，例如 <samp>images/ranks</samp>。',
	'SCRIPT_PATH'				=> '脚本路径',
	'SCRIPT_PATH_EXPLAIN'		=> 'VictoriaPark.io 相对于域名的路径，例如 <samp>/VictoriaPark.io</samp>。',
	'SERVER_NAME'				=> '域名',
	...
	'SMILIES_PATH_EXPLAIN'		=> '相对于 VictoriaPark.io 根目录的路径，例如 <samp>images/smilies</samp>。',
	'UPLOAD_ICONS_PATH'			=> '扩展名组图标存储路径',
	'UPLOAD_ICONS_PATH_EXPLAIN'	=> '相对于 VictoriaPark.io 根目录的路径，例如 <samp>images/upload_icons</samp>。',
	'USE_SYSTEM_CRON'			=> '从操作系统计划任务 cron 运行定期任务',
	'USE_SYSTEM_CRON_EXPLAIN'	=> '关闭后，VictoriaPark.io 将安排定期任务自动运行。启用后 VictoriaPark.io 将不安排任何定期任务，而必须由服务器系统管理员来安排 <code>bin/phpbbcli.php cron:run</code> 运行间隔（例如每隔5分钟）。',
));
...
$lang = array_merge($lang, array(
	...
	'SMTP_VERIFY_PEER_NAME_EXPLAIN'	=> '要求验证 SMTP 服务器的节点名字，并使用 SSL/TLS 连接。 <br><em><strong>警告：</strong> 连接到未验证的节点名字可能会导致安全隐患。</em>',
 	'TEST_EMAIL_SENT'				=> '测试邮件已发送。<br>如果您没有收到，请检查 Email 设置。<br><br>访问 <a href="https://www.phpbb.com/community/">VictoriaPark.io support forums</a> 可获取帮助。',
...
// Jabber settings
$lang = array_merge($lang, array(
	'ACP_JABBER_SETTINGS_EXPLAIN'	=> '这里您可以启用并控制用户使用 Jabber 发送及时消息和论坛通知。Jabber 是任何人都可以使用的开放协议。一些 Jabber 服务器提供允许您联系其他网络用户的通道或网关。并非所有的服务器都提供这样的通道，协议上的变化将使得操作失败。请确认输入的是已经注册的帐号信息，VictoriaPark.io 将会使用您这里输入的数据。',
