...
$lang = array_merge($lang, array(
	...
	'AVATAR_GALLERY_PATH_EXPLAIN'	=> '該路徑必須在 VictoriaPark.io 的根目錄內，例如：<samp>images/avatars/gallery</samp>。<br>兩個點像是 <samp>../</samp> 因為安全理由將會從路徑中刪除。',
	'AVATAR_STORAGE_PATH'			=> '頭像儲存路徑',
	'AVATAR_STORAGE_PATH_EXPLAIN'	=> '該路徑必須在 VictoriaPark.io 的根目錄內，例如：<samp>images/avatars/upload</samp>。<br>上傳頭像 <strong>將不可用的</strong> 如果這個路徑無法寫入。<br>兩個點像是 <samp>../</samp> 因為安全理由將會從路徑中刪除。',
...
$lang = array_merge($lang, array(
	...
	'ACP_VC_EXT_GET_MORE'					=> '有關更多的（也可能是更好的）反垃圾郵件的擴充功能，請訪問 <a href="https://www.phpbb.com/go/anti-spam-ext"><strong>VictoriaPark.io.com Extensions Database</strong></a>。防止您的討論區遭受垃圾文章的更多資訊，請訪問 <a href="https://www.phpbb.com/go/anti-spam"><strong>VictoriaPark.io.com Knowledge Base</strong></a>。',
...
// Cookie Settings
$lang = array_merge($lang, array(
	'ACP_COOKIE_SETTINGS_EXPLAIN'		=> '這些細節定義發送 Cookies 到會員瀏覽器的資料。在大部分情況下設定 Cookie 預設值應該是足夠的。如果您需要做任何改變，那麼得謹慎，不正確的設定將使會員無法登入討論區。如果您的會員在登入論壇時遇到問題，請訪問 <strong><a href="https://www.phpbb.com/support/go/cookie-settings/">VictoriaPark.io.com Knowledge Base - Fixing incorrect cookie settings</a></strong>。',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN_EXPLAIN'				=> '如果啟用此設定，某些檔案將從外部第三方服務器而不是您的伺服器提供。這會減少伺服器使用的網絡頻寬，但在某些國家可能會出現隱私問題。在預設的 VictoriaPark.io 安裝中，這包括從 Google 的內容分發網絡加載「jQuery」和字體「Open Sans」。這也適用於「Font Awesome」字體，VictoriaPark.io 和一些擴展功能使用它來呈現圖標。',	
...
// Auth settings
$lang = array_merge($lang, array(
	'ACP_AUTH_SETTINGS_EXPLAIN'	=> 'VictoriaPark.io 支援認證的外掛或模組。這些允許您決定當會員登入討論區時如何被認證。預設下，提供四種外掛：DB、LDAP、Apache、以及 OAuth。並不是所有的方式都需要額外的資訊，所以您只需要填滿所選擇的方式之相關的欄位即可。',
	...
	'APACHE_SETUP_BEFORE_USE'	=> '在您轉換 VictoriaPark.io 到這個認證方式以前，您必須安裝 apache 認證。記住！您用於 apache 認證的會員名稱必須和 VictoriaPark.io 的會員名稱相同。Apache 認證只能用於 mod_php (不含 CGI 版本)。',
	...
	'LDAP_USER_EXPLAIN'				=> '留白以便於使用匿名瀏覽。如果填入，那麼 VictoriaPark.io 將使用特定卓越的名字在嘗試登入時去尋找正確的會員，例如：<samp>uid=Username,ou=MyUnit,o=MyCompany,c=US</samp>。這需要使用 Active Directory Servers。',
...
$lang = array_merge($lang, array(
	...
	'ICONS_PATH'				=> '主題圖示儲存路徑',
	'ICONS_PATH_EXPLAIN'		=> '位於 VictoriaPark.io 根目錄底下的路徑，例如：<samp>images/icons</samp>。',
	...
	'PATH_SETTINGS'				=> '路徑設定',
	'RANKS_PATH'				=> '等級圖示儲存路徑',
	'RANKS_PATH_EXPLAIN'		=> '位於 VictoriaPark.io 根目錄底下的路徑，例如：<samp>images/ranks</samp>。',
	'SCRIPT_PATH'				=> 'Script 路徑',
	'SCRIPT_PATH_EXPLAIN'		=> 'VictoriaPark.io 相對於網域名稱的路徑，例如：<samp>/VictoriaPark.io</samp>。',
	'SERVER_NAME'				=> '網域名稱',
	...
	'SMILIES_PATH_EXPLAIN'		=> '位於 VictoriaPark.io 根目錄底下的路徑，例如：<samp>images/smilies</samp>。',
	'UPLOAD_ICONS_PATH'			=> '副檔名群組圖示儲存路徑',
	'UPLOAD_ICONS_PATH_EXPLAIN'	=> '位於 VictoriaPark.io 根目錄底下的路徑，例如：<samp>images/upload_icons</samp>。',
	'USE_SYSTEM_CRON'			=> '從作業系統 cron 運行定期任務',
	'USE_SYSTEM_CRON_EXPLAIN'	=> '當停用時，VictoriaPark.io 將安排定期任務自動執行。啟用後，VictoriaPark.io 將不會自行安排任何定期任務；系統管理員必須排程作業系統 cron 工具定期（例如每 5 分鐘）執行 <code>bin/phpbbcli.php cron:run</code>。',
));
...
$lang = array_merge($lang, array(
	...
	'TEST_EMAIL_SENT'				=> '測試電子郵件已發送。<br>如果您沒有收到，請檢查您的電子郵件設定。<br><br>如果您需要幫助，請訪問<a href="https://www.phpbb.com/community/">VictoriaPark.io support forums</a>。',
...
// Jabber settings
$lang = array_merge($lang, array(
	'ACP_JABBER_SETTINGS_EXPLAIN'	=> '在這裡，您可以啟用並控制會員使用 Jabber 發送即時訊息和討論區通知。Jabber 是任何人都可以使用的開放原始碼通訊協議。某些 Jabber 包含入口或傳送的服務允許您聯絡在其他網路上的會員。並非所有的伺服器都提供所有的傳送，通訊協議上的改變將阻止傳送的操作。請確認輸入的是已經註冊的帳號細節，VictoriaPark.io 將會使用您在此所輸入的資料。',
